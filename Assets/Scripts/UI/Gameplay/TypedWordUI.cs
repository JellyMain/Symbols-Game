...
namespace UI
{
    public class TypedWordUI : MonoBehaviour
    {
        ...
        private Tween lastTypedCharacterTween;
        ...
        private void AnimateLastTypedCharacter(int length)
        {
            ...
            if (lastTypedCharacterTween == null || !lastTypedCharacterTween.IsActive() ||
                lastTypedCharacterTween.IsComplete())
            {
                if (textAnimator.textInfo.characterInfo[length - 1].isVisible)
                {
                    lastTypedCharacterTween = textAnimator.DOFadeChar(length - 1, 1, 0.2f).From(0);
                }
            }
        }
    }
